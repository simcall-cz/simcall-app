...
import { CATEGORY_CONFIG, CATEGORIES, PROGRESS_LEVELS } from "@/lib/lessons";
...
const CATEGORY_ICONS: Record<string, React.ElementType> = {
  Home, Key, Banknote, Scale, Map, Wrench, FileText, Star, Megaphone,
  Shield, MessageCircle, TrendingUp, UserX, Users, GraduationCap,
};
...
const COLOR_MAP: Record<string, { bg: string; border: string; text: string; node: string; nodeDone: string; line: string; pill: string; pillActive: string }> = {
  ...
  green:   { bg: "bg-green-50/60",   border: "border-green-200",   text: "text-green-700",   node: "bg-green-100 border-green-300",   nodeDone: "bg-green-500",   line: "bg-green-200",   pill: "bg-green-50 text-green-600 border-green-200",   pillActive: "bg-green-100 text-green-700 border-green-300 ring-2 ring-green-200" },
  ...
  red:     { bg: "bg-red-50/60",     border: "border-red-200",     text: "text-red-700",     node: "bg-red-100 border-red-300",     nodeDone: "bg-red-500",     line: "bg-red-200",     pill: "bg-red-50 text-red-600 border-red-200",     pillActive: "bg-red-100 text-red-700 border-red-300 ring-2 ring-red-200" },
  ...
  slate:   { bg: "bg-slate-50/60",   border: "border-slate-200",   text: "text-slate-700",   node: "bg-slate-100 border-slate-300",   nodeDone: "bg-slate-500",   line: "bg-slate-200",   pill: "bg-slate-50 text-slate-600 border-slate-200",   pillActive: "bg-slate-100 text-slate-700 border-slate-300 ring-2 ring-slate-200" },
  violet:  { bg: "bg-violet-50/60",  border: "border-violet-200",  text: "text-violet-700",  node: "bg-violet-100 border-violet-300",  nodeDone: "bg-violet-500",  line: "bg-violet-200",  pill: "bg-violet-50 text-violet-600 border-violet-200",  pillActive: "bg-violet-100 text-violet-700 border-violet-300 ring-2 ring-violet-200" },
};
...
function getCompletedCount(lessons: LessonRow[], progress: ProgressRecord[]): number {
  return lessons.filter((l) => isLessonCompleted(l.lesson_number, progress)).length;
}

function getProgressLevel(completed: number) {
  return PROGRESS_LEVELS.find((l) => completed >= l.min && completed <= l.max) || PROGRESS_LEVELS[0];
}
...
export default function LekcePage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const [subRes, progressRes, lessonsRes] = await Promise.all([
  ...
  const completedCount = useMemo(() => getCompletedCount(lessons, progress), [lessons, progress]);
  const level = useMemo(() => getProgressLevel(completedCount), [completedCount]);
  const progressPct = Math.round((completedCount / totalLessons) * 100);

  // Group lessons by category — only categories that have lessons
  ...
  // Ordered categories: only those that have lessons, in CATEGORY_CONFIG order
  const orderedCategories = useMemo(() => {
    return CATEGORIES.filter((cat) => lessonsByCategory[cat]?.length > 0);
  }, [lessonsByCategory]);
  ...
  const categoryCompletionStats = useMemo(() => {
    ...
    for (const cat of orderedCategories) {
      ...
      const completed = catLessons.filter((l) => isLessonCompleted(l.lesson_number, progress)).length;
    ...
    return stats;
  }, [lessonsByCategory, orderedCategories, progress]);
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
        ...
        <p className="text-neutral-500 max-w-md mb-6">
          {totalLessons} strukturovaných lekcí z realitní praxe s mapou postupu a sledováním progressu.
        </p>
  ...
  function renderBoardRows(categoryLessons: LessonRow[], colorKey: string) {
        ...
        <div key={rowIdx}>
          <div className="grid grid-cols-5">
            {displayRow.map((lesson, nodeIdx) => {
              ...
              const completed = isLessonCompleted(lesson.lesson_number, progress);
              const unlocked = isLessonUnlockedInCategory(lesson, categoryLessons, progress);
              const bestScore = getBestScore(lesson.lesson_number, progress);
              const isCurrent = unlocked && !completed;
              const catIdx = categoryLessons.findIndex((l) => l.lesson_number === lesson.lesson_number) + 1;

              const nextLesson = nodeIdx < 4 ? displayRow[nodeIdx + 1] : null;
              const hasRight = nextLesson !== null && nextLesson !== undefined;

              return (
                <div key={lesson.lesson_number} className="relative flex items-center justify-center h-14 sm:h-[72px]">
                  ...
                  <button
                    onClick={() => unlocked ? router.push(`/dashboard/lekce/${lesson.lesson_number}`) : undefined}
                    disabled={!unlocked}
                    className={cn(
      ...
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-sm text-neutral-500">{totalLessons} lekcí z realitní praxe — vyberte si kategorii</p>
          </div>
        </div>
      </div>
        ...
        <div className="flex justify-between mt-2">
          {PROGRESS_LEVELS.map((l, i) => (
            <span key={i} className={cn(
              ...
              completedCount >= l.min ? "text-primary-600" : "text-neutral-300"
            )}>
        ...
        {orderedCategories.map((category) => {
          ...
          return (
            ...
            >
              ...
              <span className="hidden sm:inline">{catConf?.label || category}</span>
              <span className="sm:hidden">{(catConf?.label || category).split(" ")[0]}</span>
            ...
            >
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h2 className={cn("text-sm font-bold", colors?.text)}>{catConf?.label || category}</h2>
